...
var getSpriteXml = function getSpriteXml(svgs, prefix) {
  prefix = prefix || "";
  return '<?xml version="1.0" encoding="utf-8"?>' + "\n" + '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">' + "\n" + svgs.map(function (s) {
    return '<symbol' + (s.id ? " id=\"".concat(prefix + s.id, "\"") : '') + (s.viewBox ? ' viewBox="' + s.viewBox + '"' : '') + '>' + s.content + "\n" + '</symbol>';
  }).join("\n") + '</svg>';
};

exports.getSpriteXml = getSpriteXml;

var publicWriteToFile = function publicWriteToFile(filePath, prefix, svgs) {
  return writeToFile(filePath, getSpriteXml(svgs, prefix));
};

exports.writeToFile = publicWriteToFile;

var publicWriteToConsole = function publicWriteToConsole(prefix, svgs) {
